...
namespace Microsoft.Boogie
{
  ...
  public class ExecutionEngine : IDisposable
  {
    ...
    public ExecutionEngine(ExecutionEngineOptions options, IVerificationResultCache cache, TaskScheduler scheduler, bool disposeScheduler = false) {
      Options = options;
      ProofGenerationOptions.Install(Options);  // TODO: move this Option refereence (and import + csproj dependency)
      Cache = cache;
    ...
    public async Task<bool> ProcessProgram(TextWriter output, Program program, string bplFileName, string programId = null, CancellationToken cancellationToken = default)
    {
      ...
      if (!proofGenSubsetChecker.ProofGenPotentiallySupportsSubset(program, out object resultNode))
      {
        ...
        Environment.Exit(1);
      } else if (Options.OnlyCheckProofGenSupport)
      {
      ...
      if (Options.ProofOutputDir != null)
      {
        if (Directory.Exists(Options.ProofOutputDir))
        {
          Console.WriteLine("Proof generation output directory " + Options.ProofOutputDir + " already exists ");
          Environment.Exit(1);
        }
        else
        {
          ProofGenerationOutput.CreateMainDirectory(Options.ProofOutputDir, true);
        }
      }
      else
      {
      ...
      out CivlTypeChecker civlTypeChecker)
    {
      ...
      foreach (var tuple in AstToCfgProofGenInfoManager.GetImplToProofGenInfo())
      {
        ...
        var unoptimizedBlockCopies = proofGenInfo.CopyBlocks(Options, impl.Blocks , predecessorMap, true, cmd => false, out var newVarsAfterDesugaring);
