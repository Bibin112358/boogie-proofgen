  ...
  public class Split : ProofRun
  {
        ...
        uint rlimit, CancellationToken cancellationToken)
      {
        ...
        lock (Implementation)
        {
          ...
          switch (ProofGenerationOptions.Clo.TypeEncodingMethod)
          {
            case ProofGenerationOptions.TypeEncoding.Predicates:
              typePremiseEraserFactory = new TypePremiseEraserFactory(checker.VCExprGen, bet, true);
              break;
            case ProofGenerationOptions.TypeEncoding.Monomorphic:
          ...
          vc = parent.GenerateVCAux(Implementation, null, absyIds, checker.TheoremProver.Context);
